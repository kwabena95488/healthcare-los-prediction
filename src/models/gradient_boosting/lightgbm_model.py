...
class LightGBMModel(BaseModel):
    """
    A LightGBM model for Healthcare Length of Stay Prediction.
    ...
    def __init__(self, config: Dict[str, Any]):
        """
        Initializes the LightGBMModel with LightGBM parameters
        and a preprocessing pipeline.

        Args:
            config (Dict[str, Any]): Configuration dictionary for the model.
                                      Expected keys: 'model_params' for LightGBM,
        ...
        self.model = None # LightGBM trained model

    def train(self, X_train: pd.DataFrame, y_train: pd.Series, **kwargs: Any) -> None:
        """
        Trains the LightGBM model, including fitting the preprocessor and label encoder.
        ...
        train_data = lgb.Dataset(X_train_transformed, label=y_train_encoded)
    ...
    def predict(self, X: pd.DataFrame, **kwargs: Any) -> pd.Series:
        ...
        print("Making predictions with LightGBMModel...")
        ...
        y_pred_proba = self.model.predict(X_transformed, num_iteration=self.model.best_iteration)
    ...
    def evaluate(self, y_true: pd.Series, y_pred: pd.Series, **kwargs: Any) -> Dict[str, float]:
        ...
        if log_artifacts:
            ...
            plt.tight_layout()
            cm_path = 'confusion_matrix_lgb.png'
            plt.savefig(cm_path)
            ...
            report_path = 'classification_report_lgb.csv'
            ...
            if self.model:
                ...
                # Ensure importance array matches number of features
                if len(importance) == len(feature_names_out):
                    sorted_importances = dict(sorted(zip(feature_names_out, importance), key=lambda x: x[1], reverse=True))
                else:
                    print("Warning: Feature importance length mismatch. Logging raw importance.")
                    sorted_importances = {f"feature_{i}": imp for i, imp in enumerate(importance)}
                ...
                plt.tight_layout()
                importance_path = 'feature_importance_lgb.png'
                ...
                importance_csv = 'feature_importance_lgb.csv'
                importance_df.to_csv(importance_csv, index=False)
                mlflow.log_artifact(importance_csv)
            
        return metrics

def run_lightgbm_evaluation_refactored():
    """
    Orchestrates the complete LightGBM model evaluation process using the refactored class.
    This function will simulate data loading and cross-validation until dedicated modules are ready.
    """
    print("Running refactored LightGBM evaluation...")
    ...
    # Initialize the refactored LightGBM model
    lightgbm_model = LightGBMModel(config=model_config)
    
    all_y_test = []
    all_y_pred = []
    
    # Start MLflow run
    with mlflow.start_run(run_name="Refactored_LightGBM_Model"):
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(cv_folds):
            ...
            lightgbm_model.train(X_train, y_train, fold_idx=fold_idx)
            
            # Make predictions
            y_pred = lightgbm_model.predict(X_test)
            
            # Evaluate and log metrics for the fold
            metrics = lightgbm_model.evaluate(y_test, y_pred, fold_idx=fold_idx)
        ...
        lightgbm_model.evaluate(pd.Series(all_y_test), pd.Series(all_y_pred), log_artifacts=True)
        
        # Log the model (the last trained model from the CV loop)
        mlflow.lightgbm.log_model(lightgbm_model.model, "refactored_lightgbm_model")
        
        print("\nRefactored LightGBM model evaluation completed and logged to MLflow!")

if __name__ == "__main__":
    run_lightgbm_evaluation_refactored()
